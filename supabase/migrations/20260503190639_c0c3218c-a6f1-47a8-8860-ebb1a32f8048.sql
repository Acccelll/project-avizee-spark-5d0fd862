-- RPC: atualizar_financeiro_nota
-- Atualiza forma/condição de pagamento de uma nota fiscal e regenera lançamentos no financeiro,
-- preservando parcelas já baixadas (paga/baixada/parcial).
CREATE OR REPLACE FUNCTION public.atualizar_financeiro_nota(
  p_nota_id uuid,
  p_forma_pagamento text,
  p_condicao_pagamento text,
  p_parcelas jsonb DEFAULT NULL  -- [{numero?, vencimento, valor}] (opcional p/ a_vista)
...
BEGIN
  ...
  -- Verifica baixados (não alterar lançamentos pagos/baixados/parcial)
  SELECT count(*) INTO v_baixados
    FROM public.financeiro_lancamentos
   WHERE nota_fiscal_id = p_nota_id
     AND ativo = true
     AND status IN ('pago','baixado','parcial');
  ...
  -- Inativa lançamentos abertos vinculados à nota (substituição)
  UPDATE public.financeiro_lancamentos
     SET ativo = false, updated_at = now()
   WHERE nota_fiscal_id = p_nota_id
     AND ativo = true
     AND status NOT IN ('pago','baixado','parcial');

  -- Monta plano se não veio
  ...
  FOR v_dup IN SELECT * FROM jsonb_array_elements(p_parcelas) LOOP
    ...
    INSERT INTO public.financeiro_lancamentos (
      ...
      v_idx,
      v_total,
      CASE WHEN v_nf.tipo='entrada' THEN 'nfe_entrada' ELSE 'nfe_saida' END,
      'nota_fiscal', v_nf.id,
...
REVOKE ALL ON FUNCTION public.atualizar_financeiro_nota(uuid, text, text, jsonb) FROM public;
GRANT EXECUTE ON FUNCTION public.atualizar_financeiro_nota(uuid, text, text, jsonb) TO authenticated;